    ...
    <div class="flex flex-col items-center justify-center gap-2 rounded-lg">
      ...
      <div class="flex flex-col gap-1 w-full">
        <div class="w-full flex justify-between">
          ...
          <span class="text-xs font-medium text-white">lvl {{ userLevel }}/5</span>
        </div>
        <div class="w-full rounded-full h-6 bg-gray-800">
          <div class="w-1/2 h-6 rounded-full bg-gradient-to-r from-purple-500 to-pink-400"></div>
        </div>
      </div>
    </div>

    <div class="flex flex-col items-center justify-center p-2 w-full rounded-lg">
      <span class="flex flex-row w-full justify-center items-center gap-2 mt-8">
        <p class="text-2xl text-white font-semibold">{{ raccoonsPerSec }}</p>
        <img src="/coin.png" alt="Coin Image" class="w-4 h-4" />
        <p class="text-2xl text-white font-semibold">/s</p>
      </span>
      <div class="flex flex-row w-full justify-center items-center gap-2 mt-8">
        <p class="text-2xl text-white font-semibold">{{ raccoons }}</p>
...
const userLevel = inject('userLevel');
const userStatus = ref(['hobo', 'pleb', 'baron', 'king', 'sigma']);
</script>
